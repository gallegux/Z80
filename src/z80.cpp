...
/**
	...
	http://www.myquest.com/z80undocumented
**/
...
Z80::Z80(Memory* memoria, Z80IO *io) {
    debug_z80("Z80\n");
...
u16  Z80:: getPC() { return PC; }
WORD Z80:: getAF() { return regAF; }
WORD Z80:: getBC() { return regBC; }
WORD Z80:: getDE() { return regDE; }
WORD Z80:: getHL() { return regHL; }
...
void Z80:: pc_getDatoOffset () { BYTE b; memoria->readByte(PC++, &b); instruccion_offset = (SBYTE) b; }
...
void Z80::_push(WORD reg) { SP -= 2; memoria->writeWord(SP, reg); }

void Z80::_pop(WORD* reg) { 
...
void Z80::_cp(BYTE n) {
    ...
    flag_sub = true;
    // los bits 3 y 5 se cogen del argumento
    update_flags_53(n);
}
...
void Z80:: CCF () { 
    flag_halfcarry = flag_carry; // copiar carry ant en half_carry
    flag_sub = false;
...
bool in(u8 valor, u8 limInf, u8 limSup) { return limInf <= valor && valor <= limSup; }
...
void Z80::printEstado () {
    ...
    debug_z80("[%ld %04X %04X %04X %04X %04X %04X %04X %04X [%04X] %02X %02X i:%d,%d,%d]",
        m_cycles, AF, BC, DE, HL, IX, IY, PC, SP, pila, I, R,
        IFF1_interrupcionesHabilitadas, interrupcionSolicitada, ei_ejecutado );
}


void Z80:: set_PC_SP(u16 _pc, u16 _sp) {
    PC = _pc;
    SP = _sp;
}
void Z80:: set_PC(u16 _pc) {
    PC = _pc;
}
